...
def test_nasa_topic_titles():
    ...
    print(f"\033[94mTotal article titles found:\033[0m {len(titles)}")

    # For each keyword — check how many matches
    for keyword in keywords:
        matched = [t for t in titles if keyword.lower() in t.lower()]
        print(f"\033[94mArticles containing '{keyword}':\033[0m {len(matched)}")

        if matched:
            for i, title in enumerate(matched, start=1):
                print(f"\033[94mTitle:\033[0m {i}. {title}")
        else:
            print(f"\033[94mNo articles found with keyword '{keyword}'\033[0m")
